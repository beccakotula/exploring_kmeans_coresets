...
There are `r nrow(sim.data)` in this dataset and the class distribution is: 
...
The clustering accuracy is not quite as good on this dataset as what we achieved on the Iris dataset, but we achieve nearly equivalent accuracy using only 1% of the original testing set. The total within-cluster sum of squares is still quite close as well. Now let us look more closely at the confusion matrices. 
